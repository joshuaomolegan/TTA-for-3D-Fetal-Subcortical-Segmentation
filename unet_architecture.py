...
def saveas_MHA_fun(im_array, save_path, spacing = (0.6,0.6,0.6), origin=(0, 0, 0)):
    ...
    im.SetOrigin(origin)
    sitk.WriteImage(im, str(save_path), True)
...
if __name__ == '__main__':
    ...
    # model was trained on multiple gpus, the dataparallel functions automatically takes care of that
    model_path = 'model_weights.tar'
    ...
    # test image
    path = 'test_data/01-0243_159days_1040.nii.gz'
    volume = sitk.GetArrayFromImage(sitk.ReadImage(path))
    volume_tensor = torch.zeros(1,1, *volume.shape)
    volume_tensor[0,0] = torch.Tensor(volume)

    # inference
    net.eval()
    with torch.no_grad():
        # make prediction with model
        output = net(volume_tensor.to(device))

        # activation function
        probs = torch.softmax(output, dim=1)
        predictions = torch.argmax(probs, dim = 1).squeeze()

        saveas_MHA_fun(predictions.detach().cpu().numpy(), 'test_data/01-0243_159days_1040_segm.nii.gz')
